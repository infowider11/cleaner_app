                ...
                children: [
                  ...
                  CustomTextField(
                    ...
                    suffix2: GestureDetector(
                        onTap: _toggleObscured,
                        child: Icon(_obscured==false? CupertinoIcons.eye_slash_fill : CupertinoIcons.eye_fill, color: MyColors.whiteColor,)),
                  ),
                  vSizedBox2,
                  Row(
                  ...
                  vSizedBox2,
                  RoundEdgedButton(text: 'Login',fontSize: 18,borderRadius: 8,isLoad: load,
                    color: Color(0xffffffff),textColor: MyColors.primaryColor,
                    onTap: () async{
                          ...
                          //   toast('Please enter strong password (It should contain block letter, special character & number)');
                            ...
                            if(jsonResponse['status'].toString() =="1"){
                             ...
                             if(jsonResponse['data']['type'] != null || jsonResponse['data']['type'] != ""){
                               await prefs.setString("userType", jsonResponse['data']['type']);
                               await prefs.setString("userName", jsonResponse['data']['name']);
                               await prefs.setString("userId", jsonResponse['data']['id']);
                               await prefs.setString("email", jsonResponse['data']['email']);
                               await prefs.setString("profile_image", jsonResponse['data']['profile_image']);
                               ...
                               if(jsonResponse['data']['type'].toString() == "1"){
                                 Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => HomeScreen(key:MyGlobalKeys.homeScreenKey,userType: UserType.Secratary, title: "Welcome ! ${jsonResponse['data']['name']}",)), (Route<dynamic> route) => false);
                                 toast("${jsonResponse['message']}");
                               }else if(jsonResponse['data']['type'].toString() == "2"){
                                 Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => HomeScreen(key:MyGlobalKeys.homeScreenKey,userType: UserType.Supervisor, title: "Welcome ! ${jsonResponse['data']['name']}",)), (Route<dynamic> route) => false);
                                 toast("${jsonResponse['message']}");
                               }else if(jsonResponse['data']['type'].toString() == "3"){
                                 Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => HomeScreen(key:MyGlobalKeys.homeScreenKey,userType: UserType.Logistics, title: "Welcome ! ${jsonResponse['data']['name']}",)), (Route<dynamic> route) => false);
                                 toast("${jsonResponse['message']}");
                                 ...
                                 Navigator.of(context).pushAndRemoveUntil(MaterialPageRoute(builder: (context) => HomeScreen(key:MyGlobalKeys.homeScreenKey,userType: UserType.Cleaners, title: "Welcome ! ${jsonResponse['data']['name']}",)), (Route<dynamic> route) => false);
                                 toast("${jsonResponse['message']}");
                               } else{
                                 toast("${jsonResponse['message']}");
                               }
